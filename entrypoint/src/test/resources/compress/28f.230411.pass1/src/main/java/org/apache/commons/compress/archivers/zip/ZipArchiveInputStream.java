...
public class ZipArchiveInputStream extends ArchiveInputStream {
    ...
    */
    ...
    private void processZip64Extra(ZipLong size, ZipLong cSize) {
        Zip64ExtendedInformationExtraField z64 = (Zip64ExtendedInformationExtraField) current.entry.getExtraField(Zip64ExtendedInformationExtraField.HEADER_ID);
        current.usesZip64 = z64 != null;
        if (!current.hasDataDescriptor) {
            if (// same as current.usesZip64 but avoids NPE warning
            z64 != null && (cSize.equals(ZipLong.ZIP64_MAGIC) || size.equals(ZipLong.ZIP64_MAGIC))) {
    ...
    private void closeEntry() throws IOException {
        ...
        if (current.bytesReadFromStream <= current.entry.getCompressedSize() && !current.hasDataDescriptor) {
            ...
            skip(Long.MAX_VALUE);
            long inB = current.entry.getMethod() == ZipArchiveOutputStream.DEFLATED ? getBytesInflated() : current.bytesRead;
    ...
    private boolean supportsDataDescriptorFor(ZipArchiveEntry entry) {
        return !entry.getGeneralPurposeBit().usesDataDescriptor() || (allowStoredEntriesWithDataDescriptor && entry.getMethod() == ZipEntry.STORED) || entry.getMethod() == ZipEntry.DEFLATED;
    }
    ...
    private void skipRemainderOfArchive() throws IOException {
        ...
        findEocdRecord();
        realSkip(ZipFile.MIN_EOCD_SIZE - WORD - /* signature */
        SHORT);
        readFully(SHORT_BUF);
        // file comment
