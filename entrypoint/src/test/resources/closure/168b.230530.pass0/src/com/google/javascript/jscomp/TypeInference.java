...
class TypeInference extends DataFlowAnalysis.BranchedForwardDataFlowAnalysis<Node, FlowScope> {
    ...
    private boolean ensurePropertyDeclaredHelper(Node getprop, ObjectType objectType) {
        ...
        if (qName != null) {
            ...
            if (var != null && !var.isTypeInferred()) {
                ...
                if (propName.equals("prototype") || // Handle prototype declarations that could not be addressed earlier.
    ...
    private boolean isUnflowable(Var v) {
        return v != null && v.isLocal() && v.isMarkedEscaped() && // It's OK to flow a variable in the scope where it's escaped.
