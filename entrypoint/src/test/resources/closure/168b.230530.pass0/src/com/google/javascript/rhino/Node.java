...
public class Node implements Cloneable, Serializable {
    ...
    public static final int // TODO(nicksantos): Remove this prop.
    SOURCENAME_PROP = 16, // contains a TokenStream.JSDocInfo object
    JSDOC_INFO_PROP = 29, // the name node is a variable length
    VAR_ARGS_NAME = 30, // argument placeholder.
    // pre or post type of increment/decrement
    INCRDECR_PROP = 32, // expression is parenthesized
    PARENTHESIZED_PROP = 35, // set to indicate a quoted object lit key
    QUOTED_PROP = 36, // The name node is an optional argument.
    OPT_ARG_NAME = 37, // A synthetic block. Used to make
    SYNTHETIC_BLOCK_PROP = 38, // processing simpler, and does not
    // represent a real block in the source.
    // Used to indicate BLOCK that replaced
    EMPTY_BLOCK = 39, // EMPTY nodes.
    // The original name of the node, before
    ORIGINALNAME_PROP = 40, // renaming.
    // The type syntax without curly braces.
    BRACELESS_TYPE = 41, // Function or constructor call side effect
    SIDE_EFFECT_FLAGS = 42, // flags
    // Coding convention props
    // The variable or property is constant.
    IS_CONSTANT_NAME = 43, // The parameter is optional.
    IS_OPTIONAL_PARAM = 44, // The parameter is a var_args.
    IS_VAR_ARGS_PARAM = 45, // The variable creates a namespace.
    IS_NAMESPACE = 46, // The function is a dispatcher function,
    IS_DISPATCHER = 47, // probably generated from Java code, and
    // should be resolved to the proper
    // overload if possible.
    // The ES5 directives on this node.
    DIRECTIVES = 48, // ES5 distinguishes between direct and
    DIRECT_EVAL = 49, // indirect calls to eval.
    // A CALL without an explicit "this" value.
    FREE_CALL = 50, // A StaticSourceFile indicating the file
    STATIC_SOURCE_FILE = 51, // where this node lives.
    // The length of the code represented by
    LENGTH = 52, // this node.
    // The id of the input associated with this
    INPUT_ID = 53, // node.
    // Whether a STRING node contains a \v
    SLASH_V = 54, // vertical tab escape. This is a total hack.
    // See comments in IRFactory about this.
    // Marks a function whose parameter types
    INFERRED_FUNCTION = 55, // have been inferred.
    LAST_PROP = 55;
    ...
    private static class NumberNode extends Node {
        ...
        public NumberNode(double number, int lineno, int charno) {
            ...
            throw new AssertionError("This method should not be reached! Signature: NumberNode(double, int, int)");
        }
    ...
    public void putProp(int propType, Object value) {
        if (propType == SOURCENAME_PROP) {
            putProp(STATIC_SOURCE_FILE, new SimpleSourceFile((String) value, false));
            return;
        }
        removeProp(propType);
        if (value != null) {
